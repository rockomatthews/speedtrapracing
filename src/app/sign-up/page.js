...
const SignUpPage = () => {
  ...
  const handleSocialSignUp = async (provider) => {
    try {
      ...
      console.log(`Signed up with ${provider}`, result.user);
    ...
    >
      ...
      >
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', gap: '10px', marginBottom: '20px' }}>
          <IconButton onClick={() => handleSocialSignUp('Google')} sx={{ backgroundColor: '#4285F4', color: '#fff' }}>
            <GoogleIcon />
          </IconButton>
          <IconButton onClick={() => handleSocialSignUp('Facebook')} sx={{ backgroundColor: '#3b5998', color: '#fff' }}>
            <FacebookIcon />
          </IconButton>
